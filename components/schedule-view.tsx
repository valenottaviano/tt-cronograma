          ...
          <div className="space-y-1">
            ...
            {day?.optionals && day.optionals.length > 0 && (
              <div className="pt-1 border-t border-border/40 mt-1">
                <p className="text-[10px] text-muted-foreground/50 uppercase tracking-wide mb-0.5">Opcionales</p>
                {day.optionals.map((opt, i) => (
                  <p key={i} className="text-xs text-muted-foreground">{opt.workout.name}</p>
                ))}
              </div>
            )}
          </div>
